...
from ticket_locator.services.singaporeair_service \
    import SingaporeAirService
from ticket_locator.services.transavia_service \
    import TransaviaService
from ticket_locator.services.turkishairlines_service \
    import TurkishAirlinesService
...
class SearchAirRoute(View):
    ...
    def post(self, request):
        ...
        request_post["departure_airport"] = \
            request_post["departure_airport"].split(",")[0][-4:-1]
        request_post["arrival_airport"] = \
            request_post["arrival_airport"].split(",")[0][-4:-1]
        form = SearchAirRouteForm(request_post)

        if form.is_valid():
            form.cleaned_data["departure_date"] = form.cleaned_data[
                "departure_date"
            ].strftime("%Y-%m-%d")
            if request.user.is_authenticated:
                ...
                SearchHistory.objects.get_or_create(
                    ...
                    arrival_date="2022-12-12 00:00",
                )
            request.data = form.cleaned_data
            result = FlightSearchView().post(request).data
            for flight in result:
                for flight_item in flight:
                    flight_item["AirlineLogo"] = logos.get(
                        flight_item.get("Airline")
                    )
            return render(
                request, "hello_world/index.html", {
                    "result": result,
                    "form": form
                }
            )
...
class RegistrationView(View):
    def get(self, request):
        ...
        return render(
            request=request,
            template_name="hello_world/registration.html",
            context=context,
        )
...
class Login(View):
    def get(self, request):
        ...
        return render(
            request=request,
            template_name="hello_world/login.html",
            context=context
        )
...
class UserSearchHistoryView(View):
    def get(self, request):
        ...
        return render(
            request=request,
            template_name="hello_world/history.html",
            context={"history": context, "form": search_form},
        )
