            ...
            if response:
                for flights in response:
                    ...
                    for flight_item in flights:
                        departure_time = \
                            " ".join(flight_item["DepartureTime"].split("T"))
                        arrival_time = \
                            " ".join(flight_item["ArrivalTime"].split("T"))
                        reply_text += (
                            f'Из: {flight_item["DepartureAirport"]} '
                            f'В: {flight_item["ArrivalAirport"]}\n'
                            f'Рейс: {flight_item["Airline"]}'
                            f'{flight_item["FlightNumber"]}\n'
                            f'Время вылета: {departure_time}\n'
