...
def search(update: Update, _: CallbackContext) -> None:
    ...
    if len(text) > 2:
        if re.search('[A-Za-z]{3}', text[0]) and re.search('[A-Za-z]{3}', text[1]) and re.search('[0-9]{8}', text[2]):
            request_data = {
                'departure_airport': text[0].upper(),
                'arrival_airport': text[1].upper(),
                'departure_date': text[2].upper(),
                'direct_flight': text[3] if len(text) > 3 and text[3].upper() == 'DIRECT' else None
            }
            ...
            if response:
                for flights in response:
                    ...
                    for flight_item in flights:
                        reply_text += f'Из: {flight_item["DepartureAirport"]} В: {flight_item["ArrivalAirport"]}\n' \
