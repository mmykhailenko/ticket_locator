...
class TestUserView:
    ...
    def test_get_user_by_id_empty_db(self, client):
        ...
        response = client.get(url)
        assert response.status_code == 200
        assert response.data == {"id": 1, "email": f"User with id (1) not found."}
...
class TestSearchHistoryView:
    ...
    @pytest.mark.django_db
    def test_get_history_by_user_id(self, client, create_users, create_history):
        users = create_users
        history = create_history
        for history_id, user in enumerate(users, 0):
            expected_response = [history[history_id * 2], history[history_id * 2 + 1]]
...
class TestFlightSearchView:
    ...
    @pytest.mark.parametrize("params", [
        ...
        {"departure_airport": "BCN", "arrival_airport": '', "departure_date": ''},
        {"departure_airport": "BCN", "arrival_airport": "AMS", "departure_date": ''},
    ])
    ...
    def test_post_without_direct_flight(self, client, params, mock_flight_search, flight_search_expected_response):
        url = urls.reverse("api_search")
        response = client.post(url, data=params)
...
class TestSearchAirRoute:
    ...
    @pytest.mark.parametrize("params", [
        {"departure_airport": '', "arrival_airport": '', "departure_date": ''},
        {"departure_airport": "BCN", "arrival_airport": 'OHL', "departure_date": '12456789'}
    ])
    def test_post_incorrect_data(self, client, params):
        url = urls.reverse("index")
        response = client.post(url, data=params)
    ...
    @pytest.mark.parametrize("params", [
        {"departure_airport": "Afutara Aerodrome (AFT), Bila, SB",
         "arrival_airport": "Brawley Municipal Airport (BWC), Brawley, US",
         "departure_date": '2021-12-12'},
        {"departure_airport": "Amsterdam Airport Schiphol (AMS), Amsterdam, NL",
         "arrival_airport": "Odessa International Airport (ODS), Odessa, UA",
...
class TestLogin:
    ...
    def test_post_no_such_user(self, client, user_data):
        url = urls.reverse("login")
        user = {"email": user_data["email"], "password": user_data["password1"]}
